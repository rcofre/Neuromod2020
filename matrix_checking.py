...
def fast_checking(A, tau = 0.5, reps = 100, gamma = 1):
    ...
    D_matrix = Utils.get_agreement_matrix(A, reps, tau, gamma) #Agreement matrix
    consensus_vector = Utils.consensus_und(D_matrix, tau, reps) #consensus vector 
    
    #Check integration
    global_eff = Utils.efficiency_wei(A, local = False)
    distance = Utils.distance_wei(Utils.invert(A))[0]
    charpath = Utils.charpath(distance, include_diagonal = False,
                                    include_infinite = True)[0]
    #Check segregation
    local_eff = np.mean(Utils.efficiency_wei(A, local = True))
    trans = Utils.transitivity_wu(A)        
    
    #Community and individual metrics
    BA = np.mean(Utils.participation_coef(W = A, ci = consensus_vector, degree = 'undirected'))
    Q = Utils.modularity_und(A = A, kci = consensus_vector, gamma = gamma)[1]
